...
60.000000, 180.000000, 120.000000
300.000000
60.000000, 180.000000
240.000000, 60.000000
120.000000, 60.000000
300.000000
240.000000, 300.000000
240.000000, 300.000000
240.000000
"""
...
for n in range(4, min(22, len(expected_lines))):
    ...
    print(f"n={n}: angle={angle:4}, sign={sign:2}, angle1={angle1}, expected_angle2={exp_values}")

    # Try to find pattern
    if len(exp_values) == len(angle1):
        for i in range(len(exp_values)):
            # Old pattern was: angle2 = angle1 - angle
            old_pattern = angle1[i] - angle
            while old_pattern < 0:
                old_pattern += 360

            # Try new patterns
            pattern1 = angle1[i] + angle  # angle2 = angle1 + angle
            while pattern1 < 0:
                pattern1 += 360
            while pattern1 >= 360:
                pattern1 -= 360

            pattern2 = -angle1[i] + angle  # angle2 = -angle1 + angle
            while pattern2 < 0:
                pattern2 += 360
            while pattern2 >= 360:
                pattern2 -= 360

            pattern3 = 180 - angle1[i] + angle  # angle2 = 180 - angle1 + angle
            while pattern3 < 0:
                pattern3 += 360
            while pattern3 >= 360:
                pattern3 -= 360

            diff_from_angle1 = exp_values[i] - angle1[i]
            while diff_from_angle1 < -180:
                diff_from_angle1 += 360
            while diff_from_angle1 > 180:
                diff_from_angle1 -= 360

            print(f"  [{i}]: angle1={angle1[i]:4}, angle2_exp={exp_values[i]:4}, old={old_pattern:4}, p1={pattern1:4}, p2={pattern2:4}, p3={pattern3:4}, diff={diff_from_angle1:6.1f}")
