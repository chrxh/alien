...
print("Looking at angle2 = angle1 + angleSign * X:")
print()
...
for n in range(4, min(22, len(expected_lines))):
    ...
    if len(exp_values) == len(angle1):
        for i in range(len(exp_values)):
            ...
            # Check if diff = angleSign * something
            if sign != 0:
                x = diff / sign
                print(f"  [{i}]: angle1={angle1[i]:6.1f}, angle2={exp_values[i]:6.1f}, diff={diff:6.1f}, diff/sign={x:6.1f}")
            else:
                print(f"  [{i}]: angle1={angle1[i]:6.1f}, angle2={exp_values[i]:6.1f}, diff={diff:6.1f}, sign=0")
    print()
