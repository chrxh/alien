...
public:
    ...
    __inline__ __device__ static float2 calcNormalToCell(Object* cell, float2 outward);
    __inline__ __device__ static void calcCollision(
        float2 const& vA1,
        float2 const& vB1,
        float2 const& rAPp,
        float2 const& rBPp,
    ...
    calcImpulseIncrement(float2 const& impulse, float2 relPos, float mass, float angularMass, float2& velInc, float& angularVelInc);
};
...
__device__ __inline__ float2 Physics::calcNormalToCell(Object* cell, float2 outward)
{
    ...
    for (int i = 0; i < cell->numConnections; ++i) {
        ...
        if (!minCell || h < min_h) {
            minCell = cell->connections[i].object;
            minVector = u;
            min_h = h;
        }
        if (!maxCell || h > max_h) {
            maxCell = cell->connections[i].object;
            maxVector = u;
            max_h = h;
        }
    }
...
__device__ __inline__ void Physics::calcCollision(
    float2 const& vA1,
    float2 const& vB1,
    float2 const& rAPp,
    float2 const& rBPp,
    ...
    float& angularVelB2)
{
    ...
    if (angularMassA > NEAR_ZERO && angularMassB > NEAR_ZERO) {
        float j = -2.0 * vAB_dot_n / ((1.0 / massA + 1.0 / massB) + rAPp_dot_n * rAPp_dot_n / angularMassA + rBPp_dot_n * rBPp_dot_n / angularMassB);
    ...
    if (angularMassA <= NEAR_ZERO && angularMassB > NEAR_ZERO) {
        float j = -2.0 * vAB_dot_n / ((1.0 / massA + 1.0 / massB) + rBPp_dot_n * rBPp_dot_n / angularMassB);
    ...
    if (angularMassA > NEAR_ZERO && angularMassB <= NEAR_ZERO) {
        float j = -2.0 * vAB_dot_n / ((1.0 / massA + 1.0 / massB) + rAPp_dot_n * rAPp_dot_n / angularMassA);
...
Physics::calcImpulseIncrement(float2 const& impulse, float2 relPos, float mass, float angularMass, float2& velInc, float& angularVelInc)
{
