...
__global__ void cudaTestVoidUnreachableNodes(SimulationData data, uint64_t objectId)
{
    ...
    for (int index = partition.startIndex; index <= partition.endIndex; ++index) {
        ...
        if (shouldProcess) {
            GeneGraphProcessor::voidNodesUnreachableFromLastNode(data, object->typeData.cell.creature->genome);
        }
        block.sync();
    }
}
...
__global__ void cudaTestLimitGenesWithSeparation(SimulationData data, uint64_t objectId)
{
    DEVICE_CHECK(blockDim.x == NEURAL_NET_INPUTS);
    ...
    for (int index = partition.startIndex; index <= partition.endIndex; ++index) {
        ...
        if (shouldProcess) {
            GeneGraphProcessor::limitGenesWithSeparation(data, object->typeData.cell.creature->genome);
        }
        block.sync();
    }
}
