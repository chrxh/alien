...
public:
    ...
    __host__ __inline__ void init()
    {
        ...
        CHECK_FOR_DEVICE_ERRORS(cudaMemcpy(_data, &data, sizeof(T*), cudaMemcpyHostToDevice));
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_numEntries, 0, sizeof(uint64_t)));
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_numOrigEntries, 0, sizeof(uint64_t)));
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_capacity, 0, sizeof(uint64_t)));
    }
    ...
    __host__ __inline__ void init(uint64_t size)
    {
        ...
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_numEntries, 0, sizeof(uint64_t)));
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_numOrigEntries, 0, sizeof(uint64_t)));
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_capacity, size, sizeof(uint64_t)));
    }
    ...
    __host__ __inline__ void resize(uint64_t newSize) const
    {
        ...
        if (size > 0) {
            T* data;
            CHECK_FOR_DEVICE_ERRORS(cudaMemcpy(&data, _data, sizeof(T*), cudaMemcpyDeviceToHost));
            CudaMemoryManager::getInstance().freeMemory(data);
        }
        T* newData;
        CudaMemoryManager::getInstance().acquireMemory<T>(newSize, newData);
        CHECK_FOR_DEVICE_ERRORS(cudaMemcpy(_data, &newData, sizeof(T*), cudaMemcpyHostToDevice));
        CHECK_FOR_DEVICE_ERRORS(cudaMemcpy(_capacity, &newSize, sizeof(uint64_t), cudaMemcpyHostToDevice));
    }

    __host__ __inline__ T* getArray_host() const
    {
        T* result;
        CHECK_FOR_DEVICE_ERRORS(cudaMemcpy(&result, _data, sizeof(T*), cudaMemcpyDeviceToHost));
        return result;
    }
    __host__ __inline__ void setArray_host(T* data) const { CHECK_FOR_DEVICE_ERRORS(cudaMemcpy(_data, &data, sizeof(T*), cudaMemcpyHostToDevice)); }

    __host__ __inline__ uint64_t getCapacity_host() const
    {
        uint64_t result;
        CHECK_FOR_DEVICE_ERRORS(cudaMemcpy(&result, _capacity, sizeof(uint64_t), cudaMemcpyDeviceToHost));
        return result;
    }
    ...
    {
        uint64_t result;
        CHECK_FOR_DEVICE_ERRORS(cudaMemcpy(&result, _numEntries, sizeof(uint64_t), cudaMemcpyDeviceToHost));
        return result;
    }
...
public:
    ...
    __host__ __inline__ void init()
    {
        ...
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_numOrigEntries, 0, sizeof(int)));
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_numEntries, 0, sizeof(int)));
        CHECK_FOR_DEVICE_ERRORS(cudaMemset(_size, 0, sizeof(int)));
    }
