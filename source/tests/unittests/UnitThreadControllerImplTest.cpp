...
class UnitThreadControllerImplTest : public ::testing::Test
{
public:
...
TEST_F(UnitThreadControllerImplTest, testStates)
{
	...
	ASSERT_EQ(6, indexOfWorkingThreads.size()) << "Wrong number of working threads.";
	auto const msgWrongThread = "Wrong thread is working";
	ASSERT_EQ(0, indexOfWorkingThreads[0]) << msgWrongThread;
	ASSERT_EQ(3, indexOfWorkingThreads[1]) << msgWrongThread;
	ASSERT_EQ(18, indexOfWorkingThreads[2]) << msgWrongThread;
	ASSERT_EQ(21, indexOfWorkingThreads[3]) << msgWrongThread;
	ASSERT_EQ(36, indexOfWorkingThreads[4]) << msgWrongThread;
	ASSERT_EQ(39, indexOfWorkingThreads[5]) << msgWrongThread;
