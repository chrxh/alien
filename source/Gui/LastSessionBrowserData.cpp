...
void LastSessionBrowserData::load(std::unordered_set<NetworkResourceRawTO> const& rawTOs)
{
    ...
    auto lastIdentifiers = GlobalSettings::get().getStringVector(
...
void LastSessionBrowserData::save()
{
    ...
    GlobalSettings::get().setStringVector("windows.browser.last session.simulation ids", std::vector(_identifiers.begin(), _identifiers.end()));
}
