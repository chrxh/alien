...
void GettingStartedWindow::processIntern()
{
    ...
    if (ImGui::BeginChild("##", ImVec2(0, ImGui::GetContentRegionAvail().y - scale(50.0f)), false)) {
         ...
         * FIRST STEPS
         */
        drawHeading1(_("First steps"));

        ImGui::Text("%s", _("The easiest way to get to know the ALIEN simulator is to download and run an existing simulation file. You can then try out different "
        ...
        ImGui::Text("%s", _("For the beginning, however, you can use the example already loaded. Initially, it is advisable to become acquainted with the "
                    "windows for temporal and spatial controls. The handling should be intuitive and requires no deeper knowledge."));
        drawItemText(_("In the temporal control window, a simulation can be started or paused. The execution speed "
        ...
        drawItemText(_("The spatial control window combines zoom information and settings on the one hand, and "
        ...
        ImGui::Text("%s", _("There are basically two modes of how the user can operate in the view where the simulation is "
        ...
        ImGui::Text("%s", _("To be able to experiment with existing simulations, it is important to know and change the "
        ...
        drawHeading2(_("Fluids, walls and soft bodies"));
        drawParagraph(_("There are several pure physics simulations demonstrating the engines' capability. They are suitable for testing the influence of "
                      "simulation parameters such as 'Smoothing length', 'Pressure', 'Viscosity', etc."));
        drawItemText(_("Fluids/Pump with Soft-Bodies"));
        drawItemText(_("Demos/Perpetual Motion Machine"));
        drawItemText(_("Demos/Stormy Night"));

        drawHeading2(_("Evolution simulations"));
        drawParagraph(_("By attaching higher-level functions to particle networks, complex multi-cellular organisms can be modeled. They can evolve over time as "
        ...
        drawItemText(_("Deep Down Below/Selected Results"));
        drawItemText(_("Primordial Ocean/Selected Results"));
        drawItemText(_("v4.8-Evolution/Gradient/Selected Results"));
        drawItemText(_("Evolution Sandbox/Example"));
        drawItemText(_("Complex Evolution Testbed/Example"));

        drawHeading2(_("Plant-herbivore ecosystems"));
        drawParagraph(_("By customizing the cells according to their color, it is possible to specify different types of organisms. There are many examples that "
        ...
        drawItemText(_("Twin Worlds/Example"));
        drawItemText(_("Bugs and Flowers/Example"));
        drawItemText(_("Self-replicating Fluid/Initial Setting"));

        drawHeading2(_("Swarming"));
        drawParagraph(_("There are powerful sensors available as cell functions for detecting concentrations of specific colors in the surroundings. "
        ...
        drawItemText(_("Swarms/Space Invaders"));
        drawItemText(_("Evolving Swarms/Example"));

        /**
         * BASIC NOTION
         */
        drawHeading1(_("Basic notion"));

        ImGui::Text("%s", _("Generally, in an ALIEN simulation, all objects as well as thermal radiation are modeled by different types of particles moving through an "
                    "empty space. The following terms are frequently used:"));

        ImGui::Spacing();
        drawHeading2(_("World"));
        ...
        drawItemText(_("Position in space"));
        drawItemText(_("Velocity"));
        drawItemText(_("Internal energy (may be interpreted as its temperature)"));
        drawItemText(_("Upper limit of connections"));
        drawItemText(_("Living state"));

        ImGui::Spacing();
        drawHeading2(_("Cell connection"));
        ImGui::Text("%s", _(
        ...
        ImGui::Spacing();
        drawHeading2(_("Cell function"));
        ImGui::Text("%s", _("It is possible to assign a special function to a cell, which will be executed at regular time intervals. The following functions are "
                    "implemented:"));
        drawItemText(_("Neuron: It equips the cell with a small network of 8 neurons. It processes input gained from the signals of connected cells and provides an "
        ...
        drawItemText(_("Constructor: A constructor can build a cell network based on a built-in genome. The construction is done cell by cell and requires "
                     "energy. A constructor can either be controlled via signals or become active automatically (default)."));
        drawItemText(_("Injector: It can infect other constructor cells to inject its own built-in genome."));
        drawItemText(_("Nerve: On the one hand, it transfers signals from connected input cells and on the other hand, it can optionally generate "
                     "signals at specific intervals."));
        drawItemText(_("Attacker: It attacks surrounding cells from other cell networks by stealing energy from them."));
        drawItemText(_("Defender: It reduces the attack strength when another cell in the vicinity performs an attack."));
        drawItemText(_("Muscle: When a muscle cell is activated, it can produce either a movement, a bending or a change in length of the cell connection."));
        drawItemText(_("Sensor: If activated, it performs a long-range scan for the concentration of cells with a certain color."));
        drawItemText(_("Reconnector: Has the ability to dynamically create or destroy connections to other cells with a specified color."));
        drawItemText(_(
        ...
        drawParagraph(_("The process for updating the values of a signal is as follows: Firstly, the values of all input signals (i.e. "
        ...
        ImGui::Spacing();
        drawHeading2(_("Cell color"));
        drawParagraph(_("In addition to cell functions, a color can be used to perform additional user-defined customization of cells. For this purpose, most "
        ...
        ImGui::Spacing();
        drawHeading2(_("Cell network"));
        drawParagraph(_("A cell network is a connected graph consisting of cells and cell connections. Two cells in a network are therefore "
                    "connected to each other directly or via other cells. A cell network physically represents a particular body."));

        ImGui::Spacing();
        drawHeading2(_("Genome"));
        drawParagraph(_("The blueprints for entire cell networks can be stored in genomes. These genomes are translated into real cell networks by constructor "
        ...
        drawParagraph(_("A genome in ALIEN may describe several cell networks, which are hierarchically structured und possibly connected when constructed. More "
        ...
        ImGui::Spacing();
        drawHeading2(_("Energy particle"));
        drawParagraph(_(
        ...
        ImGui::Spacing();
        drawHeading2(_("Pattern"));
        drawParagraph(_("A pattern is a set of cell networks and energy particles."));

        /**
         * SIMULATION PARAMETERS
         */
        drawHeading1(_("Simulation parameters"));
        drawParagraph(_(
            ...
            "different color."));
        drawParagraph(_("Regardless of this, many parameters can also be set depending on the cell color. For this purpose click the '+' button beside the "
                      "parameter. This customization is useful when you want to define different classes of species."));
        drawParagraph(_("Sometimes it is difficult to set precise values in a slider. In this case, you can click on the slider while holding the CTRL key. This "
                      "allows you to enter the exact value in an input field and confirm it by pressing ENTER."));
        drawParagraph(_("In general, the following types of parameters can be set."));
        drawHeading2(_("Rendering"));
        drawParagraph(_(
        ...
        drawHeading2(_("Physics"));
        drawParagraph(_("Basic physical properties can be modified in these settings. This includes adjusting the radiation intensity, various thresholds, and "
        ...
        drawHeading2(_("Radiation sources"));
        drawParagraph(_("Optionally, you can define radiation sources by opening the corresponding editor. Typically, all cells lose energy over time by "
        ...
        drawHeading2(_("Cell specific parameters"));
        drawParagraph(_("These parameter types are particularly important when simulating (self-replicating) agents composed of cell networks, going beyond pure "
        ...
        drawParagraph(_("The mutation rates influence the probability of modifying a genome for the underlying cells. When adjusting these rates, it should "
        ...
        drawHeading2(_("Creator"));
        drawParagraph(_("If you want to create individual cells, cell networks, or energy particles, you can open the 'Creator' window. In this window you also "
        ...
        drawHeading2(_("Pattern editor"));
        drawParagraph(_("Physical properties of already selected cells and energy particles, such as center velocities, positions, colors, etc., can be "
                      "conveniently changed in the 'Pattern editor'. In addition, selections can be saved, loaded, copied and pasted."));
        drawHeading2(_("Genome editor"));
        drawParagraph(_("In the 'Genome editor', genomes describing cell networks can be created and modified. Each tab shows the principal part of the genome. "
        ...
        drawParagraph(_("To translate a genome into an actual structure, there are two options:"));
        drawItemText(_("You can generate a spore using the corresponding toolbar button. A spore is a single constructor cell containing the specific genome and "
        ...
        drawHeading2(_("Object inspection"));
        drawParagraph(_("Nearly every property of each particle can be viewed and edited. For this purpose, special editing windows can be attached to a "
        ...
        drawHeading2(_("Mass operations"));
        drawParagraph(_("Various mass editing functions are available. On the one hand, the pattern editor allows to change different physical properties of an "
        ...
        drawParagraph(_("In addition, simulations can be scaled up by increasing the size of the world and filling the resulting empty space with copies of the "
         ...
         */
        drawHeading1(_("Frequently asked questions"));

        drawHeading2(_("How does a simple self-replicating organism work?"));
        drawParagraph(_("In general, an organism in ALIEN consists of a network of cells where the cells work together by communicating with each other through "
                      "signals."));
        drawParagraph(_(
        ...
        drawParagraph(_("To perform movements, an organism requires muscle cells. These are also controlled by signals. Muscle cells can work in "
                      "various modes: they can bend, contract/expand, or generate an impulse."));
        drawParagraph(_("For the perception of the environment, sensor cells are available. When such a cell is triggered by a signal, it provide "
        ...
        drawHeading2(_("Why does the radiation source generates no energy particles?"));
        drawParagraph(_("The principle of energy conservation holds true in a simulation, which means that energy particles cannot spontaneously come into "
        ...
        drawParagraph(_("The existence of matter in the form of cells is a prerequisite for the radiation source to emit particles. Furthermore, the simulation "
                      "parameters should be adjusted in a way that guarantees a gradual loss of energy from cells over time."));

        drawHeading2(_("How can neural networks be incorporated?"));
        drawParagraph(_("Neural networks are available as cell functions. If a cell is assigned the function 'Neuron', it possesses a small neural network "
        ...
        drawParagraph(_("Furthermore, these networks can be fed by sensors and, in turn, control muscle and attacker cells."));

        drawHeading2(_("For how long should I run a simulation to see evolutionary changes?"));
        drawParagraph(_(
        ...
        drawHeading2(_("How can I add energy to a simulation?"));
        drawParagraph(_("Adding energy to a simulation can increase the available resources and thus the population. There is a convenient way to directly feed "
        ...
        drawHeading2(_("How can I create a cell signal in the first place?"));
        drawParagraph(_("To activate most cell functions, an input from a connected cell in the form of a signal is required. The simplest methods "
                      "to generate a signal are as follows:"));
        drawItemText(_("The most direct approach involves using a nerve cell that generates an signal at regular time intervals. The advantage here is that "
            "you can precisely configure the length of the time intervals."));
        drawItemText(_("Signals can also be generated within a neuron cell using bias values."));
        drawParagraph(_("Additionally, other cells such as constructor cells provide an output signal as soon as they are triggered (automatically)."));
