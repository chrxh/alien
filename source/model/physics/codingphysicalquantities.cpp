...
qreal CodingPhysicalQuantities::convertDataToAngle(quint8 b)
{
...
quint8 CodingPhysicalQuantities::convertAngleToData(qreal a)
{
...
qreal CodingPhysicalQuantities::convertDataToShiftLen(quint8 b)
{
	return (0.5 + (qreal)b) / 100.0;                    //add 0.5 to prevent discretization errors
}

quint8 CodingPhysicalQuantities::convertShiftLenToData(qreal len)
{
...
quint8 CodingPhysicalQuantities::convertURealToData(qreal r)
{
	if (r < 0.0)
		return 0;
	if (r > 127.0)
...
qreal CodingPhysicalQuantities::convertDataToUReal(quint8 d)
{
	return static_cast<qreal>(d);
}

quint8 CodingPhysicalQuantities::convertIntToData(int i)
{
	if (i > 127)
		return i;
	if (i < -128)
