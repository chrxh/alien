...
__inline__ __device__ void CellProcessor::aging(SimulationData& data)
{
    ...
    for (int index = partition.startIndex; index <= partition.endIndex; ++index) {
        ...
        if (cudaSimulationParameters.expertSettingsToggles.cellColorTransitionRules) {
            int transitionDuration;
            int targetColor;
            auto color = calcMod(cell->color, MAX_COLORS);
            auto spotIndex = ZoneCalculator::getFirstMatchingSpotOrBase(data, cell->pos, &SimulationParametersZoneActivatedValues::cellColorTransition);
            if (spotIndex == -1) {
...
__inline__ __device__ void CellProcessor::livingStateTransition_calcFutureState(SimulationData& data)
{
    ...
    for (int index = partition.startIndex; index <= partition.endIndex; ++index) {
        ...
        if (cell->barrier) {
            ...
            livingState = LivingState_Ready;
            if (cudaSimulationParameters.expertSettingsToggles.cellAgeLimiter && cudaSimulationParameters.resetCellAgeAfterActivation) {
