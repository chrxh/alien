...
__device__ float3 mix(float3 const& a, float3 const& b, float3 const& c, float factor1, float factor2)
{
    ...
    weight2 /= sum;
    weight3 /= sum;
    return float3{a.x * weight1 + b.x * weight2 + c.x * weight3, a.y * weight1 + b.y * weight2 + c.y * weight3, a.z * weight1 + b.z * weight2 + c.z * weight3};
}

__global__ void drawBackground(uint64_t* imageData, int2 imageSize, int2 worldSize, float zoom, float2 rectUpperLeft, float2 rectLowerRight)
{
...
__device__ __inline__ float2 mapUniversePosToPixelImagePos(float2 const& rectUpperLeft, float2 const& pos, float zoom)
{
    ...
    return {(toFloat(toInt(pos.x - toInt(rectUpperLeft.x))) - offset.x) * zoom, (toFloat(toInt(pos.y - toInt(rectUpperLeft.y))) - offset.y) * zoom};
}
...
__device__ __inline__ float3 calcColor(Cell* cell, int selected)
{
    ...
    return {
        toFloat((cellColor >> 16) & 0xff) / 256.0f * factor, toFloat((cellColor >> 8) & 0xff) / 256.0f * factor, toFloat(cellColor & 0xff) / 256.0f * factor};
}
...
{
    return selected ? float3{0.75f, 0.75f, 0.75f} : float3{0.5f, 0.5f, 0.5f};
}
...
__global__ void drawImageKernel(float2 rectUpperLeft, float2 rectLowerRight, int2 imageSize, float zoom, SimulationData data, RenderingData renderingData)
{
    ...
    DEPRECATED_KERNEL_CALL(drawCells, data.size, rectUpperLeft, rectLowerRight, data.entities.cellPointers, targetImage, imageSize, zoom);

    DEPRECATED_KERNEL_CALL(drawTokens, data.size, rectUpperLeft, rectLowerRight, data.entities.tokenPointers, targetImage, imageSize, zoom);

    DEPRECATED_KERNEL_CALL(drawParticles, data.size, rectUpperLeft, rectLowerRight, data.entities.particlePointers, targetImage, imageSize, zoom);
