...
bool _MaxAgeBalancer::balance(SimulationParameters& parameters, StatisticsData const& statistics, uint64_t timestep)
{
    auto result = false;
...
bool _MaxAgeBalancer::doAdaptionIfNecessary(SimulationParameters& parameters, StatisticsData const& statistics, uint64_t timestep)
{
    auto result = false;
    for (int i = 0; i < MAX_COLORS; ++i) {
