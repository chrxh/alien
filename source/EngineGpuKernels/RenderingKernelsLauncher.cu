    ...
    RenderingData renderingData)
{
    ...
    KERNEL_CALL(cudaDrawBackground, targetImage, imageSize, data.worldSize, zoom, renderScale, rectUpperLeft, rectLowerRight);
    ...
    KERNEL_CALL(
        cudaDrawCells, data.timestep, data.worldSize, rectUpperLeft, rectLowerRight, data.tempObjects.cellPointers, targetImage, imageSize, zoom, renderScale);
    KERNEL_CALL(
        cudaDrawParticles, data.worldSize, rectUpperLeft, rectLowerRight, data.tempObjects.particlePointers, targetImage, imageSize, zoom, renderScale);
    if (settings.simulationParameters.showRadiationSources) {
        KERNEL_CALL_1_1(cudaDrawRadiationSources, targetImage, rectUpperLeft, data.worldSize, imageSize, zoom, renderScale);
    }
    ...
    if (settings.simulationParameters.features.cellGlow) {
        ...
        cudaDrawCellGlow<<<blocks, threadsPerBlock>>>(
            data.worldSize, rectUpperLeft, data.tempObjects.cellPointers, targetImage, imageSize, zoom, renderScale);
    }
