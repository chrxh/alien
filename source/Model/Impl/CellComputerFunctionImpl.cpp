...
QString CellComputerFunctionImpl::decompileInstructionCode () const
{
    QString text;
...
CellComputerFunction::CompilationState CellComputerFunctionImpl::injectAndCompileInstructionCode (QString sourceCode)
{
	...
	for (int bytePos = 0; bytePos < sourceCode.length(); ++bytePos) {
        ...
        if( instructionUncoded.readingFinished ) {
			linePos++;
			if (!resolveInstructionAndReturnSuccess(_context->getSymbolTable(), instructionCoded, instructionUncoded)) {
				return{ false, linePos };
			}
            writeInstruction(_code, instructionCoded);
