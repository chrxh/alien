...
namespace {
	...
	{
		if (CudaPhysics::compareEqualAngle(beforeAngle, afterAngle, FLOATINGPOINT_LOW_PRECISION))
	...
	{
		if (CudaPhysics::compareEqualAngle(calculatedRelativeAngle, expectedRelativeAngle, FLOATINGPOINT_LOW_PRECISION))
			return ::testing::AssertionSuccess();
		else
...
TEST_F (PhysicsTest, testRotateClockwise)
{
    ...
    QVector2D v1r = CudaPhysics::rotateClockwise(v1, 90.0);
    QVector2D v2r = CudaPhysics::rotateClockwise(v2, 90.0);
    QVector2D v3r = CudaPhysics::rotateClockwise(v3, 90.0);
    QVector2D v4r = CudaPhysics::rotateClockwise(v4, 90.0);
...
{
    qreal a = CudaPhysics::angleOfVector(QVector2D(0.0, -1.0));
	ASSERT_PRED2(predEqualLowPrecision, 0.0, a);
    a = CudaPhysics::angleOfVector(QVector2D(1.0, 0.0));
	ASSERT_PRED2(predEqualLowPrecision, 90.0, a);
    a = CudaPhysics::angleOfVector(QVector2D(0.0, 1.0));
...
TEST_F(PhysicsTest, testUnitVectorOfAngle)
{
    ...
    for(int i = 0; i < 100; ++i) {
        ...
        QVector2D v = CudaPhysics::unitVectorOfAngle(angleBefore);
        qreal angleAfter = CudaPhysics::angleOfVector(v);
		ASSERT_PRED_FORMAT3(predUnitVectorOfAngle, angleBefore, v, angleAfter);
    }

    //test overrun
    for(qreal a = 0.0; a < 360.0; a += 10.0) {
        QVector2D v1 = CudaPhysics::unitVectorOfAngle(a);
        QVector2D v2 = CudaPhysics::unitVectorOfAngle(a+360.0);
        QVector2D v3 = CudaPhysics::unitVectorOfAngle(a-360.0);
...
TEST_F(PhysicsTest, testClockwiseAngleFromFirstToSecondVector)
{
    for(int i = 0; i < 100; ++i) {
        ...
        QVector2D v1 = CudaPhysics::unitVectorOfAngle(angle);
        QVector2D v2 = CudaPhysics::unitVectorOfAngle(angle+angleIncrement);
        qreal returnedAngle = CudaPhysics::clockwiseAngleFromFirstToSecondVector(v1, v2);
