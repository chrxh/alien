...
public:
    ...
    __inline__ __device__ static float2 mul(float2 const &p, float r);
    __inline__ __device__ static float2 div(float2 const &p, float r);
    __inline__ __device__ static float2 add(float2 const &p, float2 const &q);
    __inline__ __device__ static float2 sub(float2 const &p, float2 const &q);
...
__inline__ __device__ void Math::rotationMatrix(float angle, float(&rotMatrix)[2][2])
{
...
__inline__ __device__ void Math::inverseRotationMatrix(float angle, float(&rotMatrix)[2][2])
{
    float sinAngle = __sinf(angle*DEG_TO_RAD);
    float cosAngle = __cosf(angle*DEG_TO_RAD);
    rotMatrix[0][0] = cosAngle;
...
__device__ __inline__ float2 Math::mul(float2 const &p, float r)
{
    return{ p.x * r, p.y * r };
}

__device__ __inline__ float2 Math::div(float2 const &p, float r)
{
    return{ p.x / r, p.y / r };
}

__device__ __inline__ float2 Math::add(float2 const &p, float2 const &q)
{
    return{ p.x + q.x, p.y + q.y };
}

__device__ __inline__ float2 Math::sub(float2 const &p, float2 const &q)
{
    return{ p.x - q.x, p.y - q.y };
}
